...
public class Combatant
{
    ...
    public int GetMagicDefense()
    {
        return isPlayerCharacter ?
            (characterData != null ? characterData.MagicDefense : 0) :
            (enemyData != null ? enemyData.baseMagicDefense : 0);
    }
...
                    EnemyCombatStatusUI statusUIInstance = null;
    ...
    private IEnumerator PerformSkillSequenceForEnemy(Combatant attacker, Combatant directTarget, AbilityData skill)
    {
        ...
        foreach (Combatant t in actualTargets)
        {
            ...
            {
                int damage = Mathf.Max(1, (int)skill.power + attacker.enemyData.baseMagicAttack - t.GetMagicDefense());
                t.TakeDamage(damage);
            }
    ...
    private IEnumerator PerformAttackSequence(Combatant attacker, Combatant target)
    {
        ...
        else
        {
            ...
            StartCoroutine(EndPlayerActionAndProceedToNextTurn(skillProjectileImpactDelay));
        }
        ...
        else // Si NO es un proyectil lanzado por evento, aplicar efectos directos ahora
        {
            ...
            else
            {
                ...
                foreach (Combatant t in actualTargets)
                {
                    ...
                    if (skill.effectType == AbilityEffectType.Damage)
                    {
                        // --- LÍNEA CORREGIDA Y CON DEPURACIÓN ---
                        Debug.Log($"--- CÁLCULO DE DAÑO MÁGICO PARA {attacker.GetName()} ---");
                        Debug.Log($"Poder de la habilidad ({skill.abilityName}): {skill.power}");
                        Debug.Log($"Ataque Mágico del personaje ({attacker.characterData.characterName}): {attacker.characterData.MagicAttack}");
                        Debug.Log($"Defensa Mágica del objetivo ({t.GetName()}): {t.GetMagicDefense()}");
                        int damage = Mathf.Max(1, (int)skill.power + attacker.characterData.MagicAttack - t.GetMagicDefense());
                        Debug.Log($"Daño final calculado: {damage}");
                        Debug.Log("-------------------------------------------------");
                        t.TakeDamage(damage);
                    }
