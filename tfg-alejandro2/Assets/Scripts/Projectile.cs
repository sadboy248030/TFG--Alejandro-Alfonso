        ...
        if (_targetCombatant != null && _targetCombatant.combatSpriteGO == other.gameObject)
        {
            ...
            if (_originatingAbility != null && _attacker != null) // Si fue lanzado por una HABILIDAD
            {
                if (_originatingAbility.effectType == AbilityEffectType.Damage)
                {
                    ...
                    Debug.Log($"--- CÁLCULO DE DAÑO MÁGICO (PROYECTIL) PARA {_attacker.GetName()} ---");
                    Debug.Log($"Poder de la habilidad ({_originatingAbility.abilityName}): {_originatingAbility.power}");
                    Debug.Log($"Ataque Mágico del personaje ({_attacker.characterData.characterName}): {_attacker.characterData.MagicAttack}");
                    Debug.Log($"Defensa Mágica del objetivo ({_targetCombatant.GetName()}): {_targetCombatant.GetMagicDefense()}");

                    int calculatedDamage = Mathf.Max(1, (int)_originatingAbility.power + _attacker.characterData.MagicAttack - _targetCombatant.GetMagicDefense());

                    Debug.Log($"Daño final calculado: {calculatedDamage}");
                    Debug.Log("-------------------------------------------------");
