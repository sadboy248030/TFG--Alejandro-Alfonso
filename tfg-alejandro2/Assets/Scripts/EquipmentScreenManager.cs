...
public class EquipmentScreenManager : MonoBehaviour
{
    ...
    private void UpdateCharacterStatsDisplay()
    {
        ...
        // Asumiendo que tu Character.cs tiene estas propiedades
        statsBuilder.AppendLine("A.Magico: " + _currentlyDisplayedCharacter.MagicAttack);
        statsBuilder.AppendLine("D.Magico: " + _currentlyDisplayedCharacter.MagicDefense);
        statsBuilder.AppendLine("Velocidad: " + _currentlyDisplayedCharacter.Speed);
    ...
    private void DisplayItemInfoAndActions(ItemData item, RectTransform clickedUITransform, bool isFromEquipmentSlot)
    {
        ...
        if (itemStatsText_InfoPanel != null)
        {
            if (item.isEquipable)
            {
                ...
                if (item.magicAttackBonus != 0) statsBuilder.AppendLine("Ataque Mágico: " + item.magicAttackBonus);
                if (item.magicDefenseBonus != 0) statsBuilder.AppendLine("Defensa Mágica: " + item.magicDefenseBonus);
